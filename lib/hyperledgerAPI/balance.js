...
const balance = async (userId, lineBotToken) => {
    ...
    for(const i in allBalance){
        if(allBalance.length==0){
            ...
            const image_template_temp_string = JSON.stringify({
            ...
            let image_template_string = JSON.stringify(image_template, (k, v) => {
                if(k==='columns'){
                    console.log(`columns = ${v.toString().replace(']', ','+ image_template_temp_string +']')}`);
                    return v.toString().replace(']', ','+ image_template_temp_string +']');
                } else {
                    return v;
                }
            })
            console.log(`image_template = ${image_template}`);
            image_template = JSON.parse(image_template_string);
        }
    }
