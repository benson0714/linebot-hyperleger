...
const balance = async (userId, lineBotToken) => {
    await allbalance.allbalance(userId).then(async (allBalance)=>{
        ...
        for(const i in allBalance){
            ...
            await uri.uri(allBalance[i]['id']).then((uri_tmp)=>{
                uri_tmp.then((uri_tmp_ooo)=>{
                    console.log(uri_tmp_ooo['image']);
                    image_url.push(uri_tmp_ooo['image']);
                    tokenName.push(uri_tmp_ooo['name']);
                    return;
                })
                return;
            });
