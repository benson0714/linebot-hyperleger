...
const balance = async (userId, lineBotToken) => {
    ...
    for(const i in allBalance){
        if(allBalance.length==0){
            ...
            const image_template_string = JSON.stringify({
            ...
            const image_template_temp = JSON.parse(image_template_string);

              image_template['template']['columns'][i] = image_template_temp;
        }
    }
