            ...
            return await metadata.metadata(tokenId).then(async (metadata_json) => {
                if (Number(total_amount) < Number(amount)) {
                    ...
                    if (updateCheckJwtToken === "expireMessage") {
                        ...
                        const tokenName = metadata_json['name'];
                                ...
                                "contents": [
                                    {
                                        ...
                                        "action": {
                                            "type": "uri",
                                            "label": "移轉",
                                            "uri": `https://liff.line.me/${process.env.MY_LIFF_ID_TRANSFER}?address=${address}&tokenId=${tokenId}&amount=${amount}&jwtToken=${jwtToken}`
                                        },
                                        "style": "primary",
                                        "color": "#0000ff"
                                    }
