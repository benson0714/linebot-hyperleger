...
const check_amount = async (userId, lineBotToken, amount, tokenId) => {
    const total_amount = await balance.balance(userId, tokenId);
    let tokenName = '';
    await uri.uri(tokenId).then(async (uri_tmp)=>{
    ...
    if(total_amount<amount){
        ...
        flexMessage['contents']['contents']=[flex_template_temp_string];
        return flexMessage;
    }
}
