...
const history = async (userId, tokenId, tokenName, lineBotToken, replyToken) => {
  ...
  for (const i in recordJson) {
    if (i == 10) {
          ...
          "contents": [
              {
                  ...
                  "action": {
                      "type": "uri",
                      "label": "查看更多",
                      "uri": `https://liff.line.me/${process.env.MY_LIFF_ID_RECORD_DETAILS}?tokenId=${tokenId}&tokenName=${tokenName.replace(" ","-")}`
                  },
                  "style": "primary",
                  "color": "#0000ff"
              }
      ...
      } else if(recordJson[i]['to'] === address) {
        // from 不是自己就是轉入
        transfer_option = "轉入";
        transfer_address = recordJson[i]['from'];
      } else{
        transfer_option = "Error";
        transfer_address = "Error";
      }
