const pushCarouselFlex = require('./../lineAPI/pushCarouselFlexMessage.js');
    ...
    for(const i in recordJson){
        if(i>10){
            ...
            const flex_template_temp_string = {
              ...
              const temp = JSON.stringify(flex_template_temp_string)
              console.log(temp);
            flex_template['contents']['contents'].push(temp);
        }
    }
    try{
        console.log(flex_template);
        console.log(JSON.parse(flex_template));
        let message = await pushCarouselFlex.pushCarouselFlex(lineBotToken, userId, flex_template);
        return message;
    }catch(err){
        return err;
    }
