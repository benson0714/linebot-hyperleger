...
const myWallet = async (userId, lineBotToken) => {
    ...
    const address = await address_func.address(userId).then(async (address)=>{
        ...
        console.log(`flexMessage['contents']['hero']['url'] = ${flexMessage['contents']['hero']['url']}`)
        //concat flexQrcodeMessage into json
        flexMessage['contents']['hero']['url'] = await flexMessage['contents']['hero']['url'].concat(address);
        flexMessage['contents']['body']['contents'][2]['contents'][0]['contents'][1]['text'] = await address;
        flexMessage['contents']['footer']['contents'][0]['action']['data'] = await flexMessage['contents']['footer']['contents'][0]['action']['data'].concat(address)
        return flexMessage;
    })
