...
const transfer = async (userId, lineBotToken, recipient, tokenId, amount, jwtToken, replyToken) => {
    // replyMessage = [
    //     {
    //         type: 'text',
    //         text: `處理中，請稍後`
    ...
    if (updateCheckJwtToken === "expireMessage") {
        ...
        const message = replyMessageAPI.replyMessage(replyMessage, replyToken, lineBotToken);
        return;
        ...
        await transfer_func.transfer(userId, recipient, tokenId, amount).then(async (status) => {
            ...
            if (status === "error") {
                ...
                replyMessageAPI.replyMessage(message, replyToken, lineBotToken);
            } else {
                return await uri.uri(tokenId).then(async (uri_tmp) => {
                    ...
                    replyMessageAPI.replyMessage(lineBotToken, replyToken, flexMessage);
                    return flexMessage;
                })
