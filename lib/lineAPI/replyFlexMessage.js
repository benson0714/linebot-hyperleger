 ...
 */
const postFlexMessage = async (lineBotToken, userId, adderss) =>{
    ...
    try{
        ...
        let flexMessageJson = JSON.stringify(flexMessage, (k, v) => {
            if(k=='url'){
                const value = v.concat(adderss);
                return value;
            } else if(k=='contents'){
                const value = JSON.stringify(v, (k_contents, v_contents) => {
                    if(k_contents=='text'){
                        value = v_contents.concat(address);
                        return value;
                    } else{
                        return v_contents;
                    }
