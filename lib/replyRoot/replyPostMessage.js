...
const replyPostback = async (events, lineBotToken) => {
    ...
    if(action[1] === 'show_address'){ 
        ...
        const message = replyMessage.replyMessage(replyMessageData, replyToken, lineBotToken);
        return message;
        ...
        const message = await myWallet.myWallet(userId, lineBotToken, replyToken);
        return message;
    } else if (action[1] === 'transfer_balance') {
        const message = await balanceFunc.balance(userId, lineBotToken, replyToken);
        return message;
    } else if(action[1] === 'carousel_history') {
        const tokenId = string[1].split('=')[1];
        console.log(`tokenId = ${tokenId}`);
        const tokenName = string[2].split('=')[1];
        const message = history.history(userId, tokenId, tokenName, lineBotToken, replyToken);
        return message;
    } else if(action[1] === 'tf') {
        ...
        const message = transfer.transfer(userId, lineBotToken, address, tokenId, amount, jwtToken, replyToken);
        return message;
        ...
        const message = details.details(tokenId, tokenName, balance, lineBotToken, replyToken);
        return message;
    }
}
