...
const replyFlexMessage = require('./../requestServer/replyFlexMessage.js');
...
const replyPostback = async (events, lineBotToken) => {
    ...
    if (action[1] === 'replyFlexMessage') {
        ...
        replyFlexMessage.postFlexMessage(lineBotToken, userId);
    } else if(action[1] === 'template') {
        console.log('sucess enter template');
        replyTemplateMessage.postTemplateMessage(lineBotToken, userId);
